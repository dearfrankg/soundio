      ...
      .then((json) => {
        const collection = json.collection
          ...
          .filter((song) => song.kind === 'track'
            && song.streamable                    // eslint-disable-line
            && !(song.id in streamSongsMap)       // eslint-disable-line
            && !(song.id in newStreamSongsMap))   // eslint-disable-line
...
export function toggleLike (songId) {
  return (dispatch, getState) => {
    ...
    if (!(songId in likes)) {
      ...
      && selectedPlaylists[selectedPlaylists.length - 1] === `likes${AUTHED_PLAYLIST_SUFFIX}`) { // eslint-disable-line
